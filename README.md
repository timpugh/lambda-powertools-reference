...
The Pydantic models and route type hints that power `enable_validation=True` also drive an OpenAPI 3 spec. The spec is generated at **documentation-build time** by `docs/generate_openapi.py`, which imports the Lambda resolver, calls `app.get_openapi_json_schema(...)`, and writes `docs/openapi.json`. It is then rendered in the browser by [Scalar](https://github.com/scalar/scalar)'s standalone bundle via `docs/api.html`, which Sphinx copies into the built site verbatim through `html_extra_path`.
